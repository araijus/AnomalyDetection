...
def exp_map(x):
    ...
    return tf.tanh(norm) * x / (norm + EPS)

def log_map(x):
    norm = tf.norm(x, axis=-1, keepdims=True)
    return tf.atanh(norm) * x / (norm + EPS)

class HyperbolicSampling(tf.keras.layers.Layer):
  def call(self, inputs):
    ...
    return exp_map(mu + tf.exp(0.5 * sigma) * epsilon)
...
def encoder_layers(inputs, latent_dim):
  ...
  mu = tf.keras.layers.Dense(latent_dim, name='latent_mu', activation=None)(x)
  sigma = tf.keras.layers.Dense(latent_dim, name ='latent_sigma', activation=None)(x)
...
def decoder_layers(inputs, conv_shape):
  ...
  """
  z_input = tf.keras.Input(shape=(LATENT_DIM,), name='decoder_input')

  # Map back from hyperbolic to Euclidean space
  z_euclidean = tf.keras.layers.Lambda(log_map)(z_input)
...
def HyperbolicVAE(encoder, decoder, input_shape):
...
def get_models(input_shape, latent_dim):
  ...
  hvae = HyperbolicVAE(encoder, decoder, input_shape=input_shape)
  return encoder, decoder, hvae

# Get the encoder, decoder and 'master' model (called vae) #input_shape=(256,626,1)
encoder, decoder, hvae = get_models(input_shape=(5,80,1), latent_dim=LATENT_DIM)
